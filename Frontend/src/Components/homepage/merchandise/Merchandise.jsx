  ...
  return (
    <div 
      ...
      className={`relative w-full cursor-pointer transition-all duration-700 ease-out transform ${
        isSelected ? 'scale-100 z-20' : 'scale-95 opacity-60 hover:opacity-80 z-10'
      }`}
    >
      ...
      {/* Main Glass Card */}
      <div className={`relative h-full bg-black/60 backdrop-blur-xl rounded-[2rem] p-4 sm:p-5 flex flex-col border transition-colors duration-500 overflow-hidden ${
        isSelected ? 'border-white/40 shadow-[0_0_50px_rgba(0,0,0,0.5)]' : 'border-white/10 shadow-2xl'
      }`}>
        
        {/* Top Badges */}
        <div className="flex justify-between items-center mb-4 relative z-20">
          <span className="px-4 py-1.5 text-[10px] sm:text-xs font-bold tracking-widest text-white uppercase bg-black/40 backdrop-blur-md rounded-full border border-white/20">
            Limited Drop
          </span>
        ...
        <div className="relative w-full h-[140px] sm:h-[180px] mb-2 sm:mb-4 flex items-center justify-center">
          ...
          <div className="absolute w-3/4 h-3/4 rounded-full blur-3xl opacity-30 pointer-events-none" style={{ backgroundColor: accentColor }}></div>
          
          <img 
            src={image} 
            alt={title}
            className={`w-full h-full object-contain drop-shadow-[0_20px_30px_rgba(0,0,0,0.8)] transition-transform duration-1000 ${
              isSelected ? 'animate-float-slow scale-110' : 'scale-100 grayscale-[30%]'
            }`}
          />
        </div>
        ...
        <div className="mt-auto relative z-20">
          ...
          <ul className="space-y-1 sm:space-y-2">
            {features.map((feature, idx) => (
              <li key={idx} className="flex items-center text-gray-200 text-xs sm:text-sm font-light tracking-wide">
  ...
  return (
    <>
      ...
      <section className="relative w-full flex flex-col font-sans text-white py-4 sm:py-8 z-10">
        ...
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none rounded-[3rem] opacity-30 mx-4 max-w-7xl lg:mx-auto mt-10" style={{ backgroundColor: merchData[selectedItem].accentColor, filter: 'blur(200px)' }}></div>
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-8 flex flex-col items-center">
          ...
          >
            ...
            <p className="text-gray-300 text-xs sm:text-sm tracking-widest uppercase font-light">
